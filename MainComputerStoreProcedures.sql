...
BEGIN
    ...
    SELECT @type1 = CASE 
                        WHEN EXISTS (SELECT * FROM ProcessorDetails WHERE product_id = @product_id1) THEN 'Processor'
                        WHEN EXISTS (SELECT * FROM MotherboardDetails WHERE product_id = @product_id1) THEN 'Motherboard'
                        ELSE 'Unknown'
                    END;

    SELECT @type2 = CASE 
                        WHEN EXISTS (SELECT * FROM ProcessorDetails WHERE product_id = @product_id2) THEN 'Processor'
                        WHEN EXISTS (SELECT * FROM MotherboardDetails WHERE product_id = @product_id2) THEN 'Motherboard'
                        ELSE 'Unknown'
                    END;

    IF @type1 = 'Processor' AND @type2 = 'Motherboard'
    BEGIN
        SELECT @compatible = CASE 
                                WHEN p.socket = m.socket THEN 1 
                                ELSE 0 
                             END
        FROM ProcessorDetails p
        JOIN MotherboardDetails m ON p.product_id = @product_id1 AND m.product_id = @product_id2;
    END
    ELSE IF @type1 = 'Motherboard' AND @type2 = 'Processor'
    BEGIN
        SELECT @compatible = CASE 
                                WHEN p.socket = m.socket THEN 1 
                                ELSE 0 
                             END
        FROM ProcessorDetails p
        JOIN MotherboardDetails m ON p.product_id = @product_id2 AND m.product_id = @product_id1;
    END
    ELSE
    BEGIN
        SET @compatible = 0;
    ...
    IF @compatible = 1
    BEGIN
        PRINT 'The products are compatible';
    END
    ELSE
    BEGIN
        PRINT 'The products are not compatible';
    END
END;
GO
